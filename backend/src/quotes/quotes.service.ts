...
export class QuotesService implements OnModuleInit {
  ...
  async onModuleInit() {
    try {
      ...
      if (count === 0) {
        this.logger.log(
          'No quotes found in database. Seeding initial quotes...',
        );
      ...
      if (count < minQuotesNeeded) {
        this.logger.log(
          `Need more quotes to avoid repetition. Fetching additional quotes...`,
        );
  ...
  async fetchNewQuotes(limit = 5): Promise<Quote[]> {
    ...
    this.logger.log(
      `Total new quotes fetched and saved: ${allFetchedQuotes.length}`,
    );
  ...
  private async fetchQuotesFromSource(
    ...
    for (let i = 0; i < limit; i++) {
      try {
        this.logger.debug(
          `Fetching quote ${i + 1}/${limit} from ${source.name}`,
        );
        ...
        if (!transformedQuote?.text || !transformedQuote?.author) {
          this.logger.warn(
            `Invalid quote data from ${source.name}:`,
            transformedQuote,
          );
          continue;
        }
        ...
        if (!existingQuote) {
          ...
          this.logger.debug(
            `Added new quote to queue: "${transformedQuote.text.substring(0, 30)}..." by ${transformedQuote.author}`,
          );
        } else {
          this.logger.debug(
            `Quote already exists: "${transformedQuote.text.substring(0, 30)}..." by ${transformedQuote.author}`,
          );
        }
    ...
    try {
      ...
      for (const quote of quotes) {
        try {
          ...
          this.logger.debug(
            `Saved quote: "${savedQuote.text.substring(0, 30)}..." by ${savedQuote.author}`,
          );
          savedQuotes.push(savedQuote);
        } catch (error) {
          if (error.code === 'P2002') {
            this.logger.debug(
              `Quote already exists: "${quote.text?.substring(0, 30)}..." by ${quote.author}`,
            );
          } else {
  ...
  async getRandomQuote(): Promise<Quote | null> {
    try {
      ...
      if (freshQuotes.length > 0) {
        ...
        if (quote) {
          this.logger.log(
            `Found quote that hasn't been used in the last 30 days: ${quote.text.substring(0, 30)}...`,
          );
        }
      } else {
        ...
        this.logger.warn(
          'All quotes have been used within the last 30 days, selecting least recently used',
        );
        ...
        if (leastRecentlyUsedQuotes.length > 0) {
          const randomIndex = Math.floor(
            Math.random() * leastRecentlyUsedQuotes.length,
          );
      ...
      if (quote) {
        ...
        this.logger.error(
          'Failed to get random quote despite having quotes in database',
        );
        return null;
      }
    } catch (error) {
  ...
  async getQuoteByType(type?: string): Promise<Quote | null> {
    ...
    try {
      ...
      if (freshCategoryQuotes.length > 0) {
        // Select a random quote from eligible quotes
        const randomIndex = Math.floor(
          Math.random() * freshCategoryQuotes.length,
        );
  ...
  async addQuoteCategory(
    ...
    try {
      ...
      this.logger.error(
        `Failed to add category to quote ID ${quoteId}:`,
        error,
      );
      return null;
    }
